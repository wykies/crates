...
use crate::{AlwaysCase, branch::BranchId, errors::ConversionError, string_wrapper};
...
impl TryFrom<actix_web::dev::ConnectionInfo> for HostId {
    ...
    fn try_from(value: actix_web::dev::ConnectionInfo) -> Result<Self, Self::Error> {
        // Prefer real ip even though it is not safe to use for security because
        // we are not using it for security just for pre-screening traffic to
        // increase the threshold required to do a DOS
